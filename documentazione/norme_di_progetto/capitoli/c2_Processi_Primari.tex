...
\begin{itemize}
	...
	\begin{itemize}
		\item \textbf{/* ... */} per i commenti che racchiudono più di una riga, andare a capo dopo la dicitura /*  per descrivere una parte di codice e poi chiudere il commento con la dicitura */;
		\item \textbf{// … } per i commenti di linea singola che devono essere separati da uno spazio dopo la dicitura //.
	\end{itemize}
	Questa dicitura non è valida per il linguaggio python in quanto non utilizza /* */ o // per la scrittura dei commenti al loro posto si usa il cancelletto {\symbol{35}}.
...
\subsubsection{Maven} \label{ProcessiPrimariStrumentiMaven}
	Maven è uno strumento di build automation$_G$, sviluppato da \textit{Apache}, utilizzato per la gestione di progetti Java. Maven si basa sul concetto di \textit{Project Object Model} (POM), ovvero un file .xml in cui sono specificate le informazioni e configurazioni necessarie allo sviluppo di un'applicazione Java. Maven, infatti, permette di configurare tutte le dipendenze e i plugin, specificati nel pom.xml, autonomamente.
	\begin{center}
