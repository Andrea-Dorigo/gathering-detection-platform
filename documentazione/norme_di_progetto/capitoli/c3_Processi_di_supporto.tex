...
\begin{itemize}
	\item viene effettuato seguendo delle procedure già definite;
...
\begin{itemize}
	\item \textbf{Walkthrough:} i vari componenti del team analizzano gli oggetti nella loro totalità per
	cercare anomalie, senza sapere inizialmente se vi siano difetti, quali e dove siano;
	\item \textbf{Inspection:} i verificatori utilizzano delle liste di controllo per fare ispezione cercando errori specifici
...
Ogni test sui requisiti, ovvero test di \textit{sistema} e \textit{accettazione}, ha questa nomenclatura:
\begin{center}
...
\begin{itemize}
	...
	\item \textbf{RS}: specifica che il test è fatto su un requisito;
	...
	\begin{itemize}
		\item[-] \textbf{O}: se il requisito è obbligatorio;
		\item[-] \textbf{D}: se il requisito è desiderabile;
		\item[-] \textbf{F}: se il requisito è facoltativo;
	\end{itemize}
	\item \textbf{codice\_requisito}: si tratta di un numero incrementale per rendere univoco il requisito;
\end{itemize}
...
La validazione ha come scopo finale quello di garantire che il software rispetti i requisiti posti e soddisfi le esigenze del proponente.
\subsection{Descrizione} \label{ProcessiDiSupportoValidazioneDescrizione}
Il processo di validazione esamina il prodotto della fase di verifica ed accerta che questo sia pienamente conforme alle attese e ai requisiti del proponente. 
...
\begin{itemize}
	...
	\item identificazione di una strategia avente procedure per la validazione in cui i test di verifica possono essere riutilizzati;
