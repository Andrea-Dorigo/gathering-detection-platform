...
\subsubsection{MQPD01 Totalità dell’implementazione} \label{QualitàDelProdottoFunzionalitàMetricheMQPD01}
...
Dove:
\begin{itemize}
	\item \textbf{T} sta per \textit{Totalità}, riferito ai requisiti$_{\scaleto{G}{3pt}}$ \textit{prestazionali} da implementare;
	\item \textbf{RnI} sta per \textit{Requisito prestazionale non Implementato};
	\item \textbf{RI} sta per \textit{Requisito prestazionale Implementato}.
\end{itemize}
I range accettabili per il risultato di \textbf{T} sono così suddivisi:
\begin{itemize}
	\item 90\% $<$ \textbf{T} $\leq$ 100\% indica che la copertura dei requisiti$_{\scaleto{G}{3pt}}$ proposti è quasi totale;
